 ...
 * @file main.c
 *
 * Application main module for NXP LPCXpresso board demo
 *
...
struct __app_state_vars {
     ...
     * Push buttons state
     */
    volatile bool push_buttons[LPCX_NUM_PUSH_BUTTONS];
...
#   define BUTTONS_READER_THREAD_PERIOD_MS 250
    ...
    bool push_buttons[LPCX_NUM_PUSH_BUTTONS];
    ...
    for ( ; ; ) {
         ...
         * Read push buttons:
         */
        lpcxpresso_push_buttons_read(push_buttons);

        for (natural_t i = 0; i < LPCX_NUM_PUSH_BUTTONS; i++) {
