...
#   endif
    ...
    /**
     * Buffer where the most recent output generated by capture_fdc_debug_printf()
     * is stored
     */
    char fdc_debug_msg_buffer[RTOS_DEBUG_MSG_BUFFER_SIZE];

    /**
     * Next position to fill in fdc_debug_msg_buffer
     */
    uint16_t fdc_debug_msg_cursor;

    /*
     * Sequence number for next debug message to store in fdc_debug_msg_buffer
     */
    uint16_t fdc_debug_msg_seq_num;

    uint32_t fdc_signature2;
#else

    /**
     * Buffer where the most recent output generated by capture_fdc_debug_printf() is
     * stored
     */
    char fdc_debug_msg_buffer[RTOS_DEBUG_MSG_BUFFER_SIZE];
...
#define FDC_INFO_INITIALIZER(_fdc_info) \
    _fdc_info = {                                                       \
        ...
        .fdc_debug_msg_cursor = 0,					\
        .fdc_debug_msg_seq_num = 0,					\
...
void capture_fdc_debug_printf(const char *fmt, ...);
